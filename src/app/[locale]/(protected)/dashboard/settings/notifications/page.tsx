...
import {
  fetchAgentNotificationPrefs,
  updateAgentNotificationPrefs,
  type AgentNotificationPreferences,
} from "@/lib/notifications";

export default function AgentNotificationSettingsPage() {
  const t = useTranslations("notificationSettings");
  const [prefs, setPrefs] = useState<AgentNotificationPreferences>({
    email_enabled: true,
  ...
  const togglePref = async (key: keyof AgentNotificationPreferences) => {
    ...
    setError(null);
    try {
      await updateAgentNotificationPrefs(updated);
      setSuccess(true);
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs divide-y divide-slate-100 overflow-hidden">
        ...
        <div className="p-5 flex items-center justify-between gap-4 hover:bg-slate-50/50 transition-colors">
          <div className="flex items-start gap-3.5">
            ...
            <div>
              <h3 className="text-sm font-medium text-slate-900">{t("teamCommissionPendingTitle")}</h3>
              <p className="text-xs text-slate-500 mt-0.5">{t("teamCommissionPendingDesc")}</p>
            </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer shrink-0">
            <input
        ...
        <div className="p-5 flex items-center justify-between gap-4 hover:bg-slate-50/50 transition-colors">
          <div className="flex items-start gap-3.5">
            ...
            <div>
              <h3 className="text-sm font-medium text-slate-900">{t("teamCommissionPaidTitle")}</h3>
              <p className="text-xs text-slate-500 mt-0.5">{t("teamCommissionPaidDesc")}</p>
            </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer shrink-0">
            <input
        ...
        <div className="p-5 flex items-center justify-between gap-4 hover:bg-slate-50/50 transition-colors">
          <div className="flex items-start gap-3.5">
            ...
            <div>
              <h3 className="text-sm font-medium text-slate-900">{t("newKolTitle")}</h3>
              <p className="text-xs text-slate-500 mt-0.5">{t("newKolDesc")}</p>
            </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer shrink-0">
            <input
              type="checkbox"
              checked={prefs.new_kol}
              onChange={() => togglePref("new_kol")}
              disabled={saving}
              className="sr-only peer"
            />
            <div className="w-10 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-emerald-600"></div>
